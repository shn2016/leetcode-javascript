 ...
 */
 ...
 //this question has to see the picture on the web page;
var readBinaryWatch = function(num) {
  let result = [];
  for(i=0; i<12; i++){
    for (j=0; j<60; j++){
      const total = getDistributions(i,4) + getDistributions(j,6)
      if(total === num){
        const string = (j<10)? (`${i}:0${j}`) : (`${i}:${j}`);
        result.push(string);
      }
    }
  }
...
function getDistributions(time, length){
  const nums = [1,2,4,8,16,32].slice(0,length);
  let temp = length-1;
  let count = 0;
  //maybe using pop() is too expensive here;
  while (time > 0){
    const value = nums[temp];
    if(time >= value){
      time -= value;
      count++
    }
