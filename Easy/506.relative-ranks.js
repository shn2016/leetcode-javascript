/*
 * @lc app=leetcode id=506 lang=javascript
 *
 * [506] Relative Ranks
 *
 * https://leetcode.com/problems/relative-ranks/description/
 *
 * algorithms
 * Easy (47.91%)
 * Total Accepted:    39.7K
 * Total Submissions: 82.8K
 * Testcase Example:  '[5,4,3,2,1]'
 *
 * 
 * Given scores of N athletes, find their relative ranks and the people with
 * the top three highest scores, who will be awarded medals: "Gold Medal",
 * "Silver Medal" and "Bronze Medal".
 * 
 * Example 1:
 * 
 * Input: [5, 4, 3, 2, 1]
 * Output: ["Gold Medal", "Silver Medal", "Bronze Medal", "4", "5"]
 * Explanation: The first three athletes got the top three highest scores, so
 * they got "Gold Medal", "Silver Medal" and "Bronze Medal". For the left two
 * athletes, you just need to output their relative ranks according to their
 * scores.
 * 
 * 
 * 
 * Note:
 * 
 * N is a positive integer and won't exceed 10,000.
 * All the scores of athletes are guaranteed to be unique.
 * 
 * 
 * 
 */
/**
 * @param {number[]} nums
 * @return {string[]}
 */

var findRelativeRanks = function(nums) {
  //can't use const a = nums here as the operation on nums will still point 
  // to a ;
  let dummy = []
  dummy = dummy.concat(nums); 

  //sort them so we know the index as rank;
  const sorted = nums.sort(function(a,b) {return b - a});

  //put back to original array, replace value with rank;
  const result = dummy.map(element =>{
    let numsIndex = sorted.indexOf(element) + 1;
    switch(numsIndex){
      case 1 : numsIndex="Gold Medal"; break;
      case 2 : numsIndex="Silver Medal"; break;
      case 3 : numsIndex="Bronze Medal"; break;
      default : break;
    }
    return numsIndex.toString();
  })

  return result;
};

