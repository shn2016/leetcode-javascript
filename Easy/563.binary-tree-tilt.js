...
// This is done from top to bottom, but do it bottom-up should save more time;
var findTilt = function(root) {
  if(!root) return 0;
  if(!root.left && !root.right) return 0;
  const left = getBranchSum(root.left);
  const right = getBranchSum(root.right);
  const tilt = Math.abs(left - right);
  return tilt + findTilt(root.left) + findTilt(root.right);
};

function getBranchSum(node){
  if(!node) return 0;
  return node.val + getBranchSum(node.left) + getBranchSum(node.right);
}
